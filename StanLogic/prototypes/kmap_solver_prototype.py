...
row_labels = ["00", "01", "11", "10"]  # x3x4
col_labels = ["00", "01", "11", "10"]  # x1x2
num_rows = num_cols = 4
...
def simplify_group_bits(bits_list):
    # Each cell in the K-map was converted to a 4-bit string by cell_to_minterm(),
    ...
    for b in bits_list[1:]:  
        # Look at each of the 4 input variables (x1, x2, x3, x4)
        for i in range(4):  
    ...
    for r in range(num_rows):
        for c in range(num_cols):
            ...
            for h, w in group_sizes:
                ...
                # Check if *all* cells in this rectangle contain 1
                # kmap[rr][cc] indexes into the K-map at row rr, column cc
                if all(kmap[rr][cc] == 1 for rr, cc in coords):
...
for g in prime_groups:
    ...
    minterms = sorted(cell_to_minterm(r, c) for r, c in coords)
...
# Output
print("Prime implicants (term -> minterms):")
