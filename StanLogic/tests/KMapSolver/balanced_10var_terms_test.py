"""
Balanced 10-Variable K-Map Terms Analysis
==========================================

Analyzes the number of terms produced by the K-Map solver for balanced
10-variable K-maps. Balanced maps have equal probability (50%) of being 1 or 0.

This test generates balanced 10-variable K-maps and examines:
...
RESULTS_CSV = os.path.join(OUTPUTS_DIR, "balanced_10var_terms_results.csv")
STATS_TXT = os.path.join(OUTPUTS_DIR, "balanced_10var_terms_stats.txt")
HISTOGRAM_PNG = os.path.join(OUTPUTS_DIR, "balanced_10var_terms_histogram.png")

# ============================================================================
# TEST DATA GENERATION
# ============================================================================

def random_balanced_output_values_10var():
    """
    Generate a balanced random 10-variable output values list.
...
def analyze_balanced_kmaps():
    """
    Generate balanced 10-variable K-maps and analyze term counts.
    ...
    print(f"Generating and analyzing {NUM_TEST_CASES} balanced 10-variable K-maps...")
    print("=" * 70)
    ...
    for test_num in range(1, NUM_TEST_CASES + 1):
        print(f"Processing test {test_num}/{NUM_TEST_CASES}...", end=" ", flush=True)
        
        # Generate balanced output values
        output_values = random_balanced_output_values_10var()
        
        # Count ones
        num_ones = sum(1 for v in output_values if v == 1)
        
        # Create solver and minimize
        solver = KMapSolver3D(10, output_values)
        ...
        # Print progress
        print(f"{num_ones:4d} ones -> {num_terms:3d} terms")
...
def compute_statistics(results):
    ...
    stats = {
        ...
        "expected_ones": 512,  # For 10-var balanced, expected is 1024 * 0.5 = 512
    }
...
def print_statistics(stats):
    ...
    print(f"Expected ones (balanced): {stats['expected_ones']} (out of 1024)")
    
    # Display frequency distribution
    print("\nTerm Count Distribution:")
    # Limit bar length for readability
    max_count = max(stats['term_distribution'].values())
    scale_factor = min(1, 50 / max_count) if max_count > 0 else 1
...
def create_histogram(results, stats):
    ...
    plt.title('Distribution of Terms in Balanced 10-Variable K-Maps (1024 cells)', 
...
def save_statistics_txt(stats, results):
    ...
    with open(STATS_TXT, 'w') as f:
        f.write("Balanced 10-Variable K-Map Terms Analysis\n")
        f.write("=" * 70 + "\n\n")
        
        f.write(f"Configuration:\n")
        f.write(f"  Random Seed: {RANDOM_SEED}\n")
        f.write(f"  Number of Test Cases: {NUM_TEST_CASES}\n")
        f.write(f"  K-Map Type: Balanced 10-variable (p=0.5 for 1s)\n")
        ...
        f.write(f"Expected ones (balanced): {stats['expected_ones']} (out of 1024)\n")
        f.write("=" * 70 + "\n\n")
        ...
        f.write(f"2. Balanced distribution means ~50% of cells are 1s (~512 ones)\n")
        ...
        f.write(f"5. 10-variable maps represent a significant computational challenge\n")
...
def main():
    ...
    print("\n" + "=" * 70)
    print("BALANCED 10-VARIABLE K-MAP TERMS ANALYSIS")
    print("=" * 70)
