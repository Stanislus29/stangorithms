...
# Set up file paths
csv_file = Path(__file__).parent.parent / "outputs" / "decay" / "analysis" / "cluster_density_analysis_4D.csv"
output_file = Path(__file__).parent.parent / "outputs" / "decay" / "analysis" / "cluster_averages4D.txt"
...
grouped = df.groupby(['num_vars', 'density']).agg({
    'num_4d_clusters': ['mean', calculate_mode, 'count'],
...
with open(output_file, 'w') as f:
    ...
    overall_by_vars = df.groupby('num_vars').agg({
        'num_4d_clusters': ['mean', calculate_mode, 'std', 'min', 'max', 'count'],
