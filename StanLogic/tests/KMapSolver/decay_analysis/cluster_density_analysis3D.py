"""
3D Cluster Density Analysis for 5-11 Variable Boolean Functions

This script analyzes the formation and information density of 3D clusters
in K-map minimization across different variable counts and function densities.

Key Metrics:
- Number of 3D clusters formed
...
RESULTS_CSV = os.path.join(OUTPUTS_DIR, "cluster_density_analysis.csv")
REPORT_PDF = os.path.join(OUTPUTS_DIR, "cluster_density_report.pdf")
LOGO_PATH = os.path.join(SCRIPT_DIR, "..", "..", "..", "images", "St_logo_light-tp.png")

os.makedirs(OUTPUTS_DIR, exist_ok=True)

# Test configuration
VARIABLE_RANGE = range(5, 16)  # 5 to 15 variables (range is exclusive of endpoint)
...
def extract_3d_clusters(solver, form='sop'):
    """
    Extract 3D clusters from the minimization process.
    
    Returns dict with cluster information:
    - patterns: list of 3D cluster patterns
    - coverage: dict mapping pattern -> list of minterms
    - total_clusters: count of 3D clusters
    ...
    # Apply 3D clustering
    minimal_3d_patterns = solver._minimize_with_3d_clustering(β, id_set)
    
    if not minimal_3d_patterns:
        return {
            'patterns': [],
            'coverage': {},
            'total_clusters': 0,
    ...
    coverage = solver.get_cluster_coordinates(list(minimal_3d_patterns))
    
    # Calculate cluster sizes
    cluster_sizes = [len(minterms) for minterms in coverage.values()]
    
    return {
        'patterns': list(minimal_3d_patterns),
        'coverage': coverage,
        'total_clusters': len(minimal_3d_patterns),
...
def run_analysis():
    ...
    print(f"\n{'='*80}")
    print(f"3D CLUSTER DENSITY ANALYSIS")
    print(f"{'='*80}")
    ...
    with open(RESULTS_CSV, 'w', newline='') as csvfile:
        fieldnames = [
            ...
            'num_3d_clusters', 'avg_cluster_size', 'max_cluster_size', 'min_cluster_size',
            'std_cluster_size', 'total_coverage', 'coverage_by_3d_percent'
        ]
        ...
        for num_vars in VARIABLE_RANGE:
            ...
            for density in DENSITY_LEVELS:
                ...
                for test_num in range(TESTS_PER_CONFIG):
                    ...
                    # Extract 3D clusters (suppress output)
                    import io
                    import contextlib
                    
                    f = io.StringIO()
                    with contextlib.redirect_stdout(f):
                        cluster_info = extract_3d_clusters(solver, form='sop')
                    ...
                    result = {
                        ...
                        'num_3d_clusters': cluster_info['total_clusters'],
                        ...
                        'coverage_by_3d_percent': coverage_percent
                    }
    ...
    for num_vars in VARIABLE_RANGE:
        ...
        for density in DENSITY_LEVELS:
            ...
            stats[num_vars][density] = {
                'mean_clusters': np.mean([r['num_3d_clusters'] for r in config_results]),
                'std_clusters': np.std([r['num_3d_clusters'] for r in config_results]),
                ...
                'mean_coverage': np.mean([r['coverage_by_3d_percent'] for r in config_results]),
                'std_coverage': np.std([r['coverage_by_3d_percent'] for r in config_results]),
            }
...
def generate_pdf_report(results, stats):
    ...
    with PdfPages(REPORT_PDF) as pdf:
        ...
        # Title
        ax.text(0.5, 0.55, "3D Cluster Density Analysis", 
        ...
        # Plot 1: Number of 3D Clusters vs Variables
        ax1 = plt.subplot(2, 2, 1)
        for density in DENSITY_LEVELS:
            means = [stats[nv][density]['mean_clusters'] 
        ...
        ax1.set_ylabel('Average Number of 3D Clusters')
        ax1.set_title('3D Cluster Formation vs Problem Size')
        ax1.legend()
        ...
        ax2.set_title('Information Density per 3D Cluster')
        ax2.legend()
        ax2.grid(True, alpha=0.3)
        
        # Plot 3: 3D Coverage Percentage
        ...
        ax3.set_ylabel('Coverage by 3D Clusters (%)')
        ax3.set_title('Effectiveness of 3D Clustering')
        ax3.legend()
        ...
        ax4.set_title('Largest 3D Cluster Formation')
        ax4.legend()
        ...
        for i, density in enumerate(DENSITY_LEVELS):
            for j, num_vars in enumerate(VARIABLE_RANGE):
                if density in stats[num_vars]:
                    cluster_matrix[i, j] = stats[num_vars][density]['mean_clusters']
        ...
        ax.set_title('Average Number of 3D Clusters Formed\\n(Heatmap)', fontsize=14, fontweight='bold')
        
        # Add colorbar
        cbar = plt.colorbar(im, ax=ax)
        cbar.set_label('Number of 3D Clusters', rotation=270, labelpad=20)
        ...
        selected_configs = [(7, 0.5), (8, 0.5), (9, 0.5)]
        ...
        for idx, (num_vars, density) in enumerate(selected_configs):
            ...
            for r in config_results:
                if r['num_3d_clusters'] > 0:
                    all_cluster_counts.append(r['num_3d_clusters'])
            
            if all_cluster_counts:
                ax.hist(all_cluster_counts, bins=15, color='steelblue', 
                       alpha=0.7, edgecolor='black')
                ax.set_xlabel('Number of 3D Clusters')
        ...
        total_clusters = sum(r['num_3d_clusters'] for r in results)
        avg_clusters_per_test = total_clusters / len(results)
        avg_coverage = np.mean([r['coverage_by_3d_percent'] for r in results])
        
        findings = [
            f"• Total tests conducted: {len(results)}",
            f"• Average 3D clusters per test: {avg_clusters_per_test:.2f}",
            f"• Average coverage by 3D clusters: {avg_coverage:.1f}%",
