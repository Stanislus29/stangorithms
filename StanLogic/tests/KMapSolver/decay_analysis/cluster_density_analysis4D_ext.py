"""
4D Cluster Density Analysis for 16-24 Variable Boolean Functions

This script analyzes the formation and information density of 4D clusters
in K-map minimization across different variable counts and function densities.

Key Metrics:
- Number of 4D clusters formed
...
RESULTS_CSV = os.path.join(OUTPUTS_DIR, "cluster_density_analysis_4D_ext.csv")
REPORT_PDF = os.path.join(OUTPUTS_DIR, "cluster_density_report_4D_ext.pdf")
LOGO_PATH = os.path.join(SCRIPT_DIR, "..", "..", "..", "images", "St_logo_light-tp.png")

os.makedirs(OUTPUTS_DIR, exist_ok=True)

# Test configuration
VARIABLE_RANGE = range(16, 19)  # 16 to 18 variables (range is exclusive of endpoint)
    ...
    for num_vars in VARIABLE_RANGE:
        ...
        for density in DENSITY_LEVELS:
            ...
            stats[num_vars][density] = {
                'mean_clusters': np.mean([r['num_4d_clusters'] for r in config_results]),
                'std_clusters': np.std([r['num_4d_clusters'] for r in config_results]),
                ...
                'mean_coverage': np.mean([r['coverage_by_4d_percent'] for r in config_results]),
                'std_coverage': np.std([r['coverage_by_4d_percent'] for r in config_results]),
            }
...
def generate_pdf_report(results, stats):
    ...
    with PdfPages(REPORT_PDF) as pdf:
        ...
        # Title
        ax.text(0.5, 0.55, "4D Cluster Density Analysis", 
        ...
        ax1.set_ylabel('Average Number of 4D Clusters')
        ax1.set_title('4D Cluster Formation vs Problem Size')
        ax1.legend()
        ...
        ax2.set_title('Information Density per 4D Cluster')
        ax2.legend()
        ...
        ax3.set_ylabel('Coverage by 4D Clusters (%)')
        ax3.set_title('Effectiveness of 4D Clustering')
        ax3.legend()
        ...
        ax4.set_title('Largest 4D Cluster Formation')
        ax4.legend()
        ...
        selected_configs = [(18, 0.5), (20, 0.5), (22, 0.5)]
        ...
        for idx, (num_vars, density) in enumerate(selected_configs):
            ...
            for r in config_results:
                if r['num_4d_clusters'] > 0:
                    all_cluster_counts.append(r['num_4d_clusters'])
            
            if all_cluster_counts:
                ax.hist(all_cluster_counts, bins=15, color='steelblue', 
                       alpha=0.7, edgecolor='black')
                ax.set_xlabel('Number of 4D Clusters')
        ...
        total_clusters = sum(r['num_4d_clusters'] for r in results)
        avg_clusters_per_test = total_clusters / len(results)
        avg_coverage = np.mean([r['coverage_by_4d_percent'] for r in results])
        
        findings = [
            f"• Total tests conducted: {len(results)}",
            f"• Average 4D clusters per test: {avg_clusters_per_test:.2f}",
            f"• Average coverage by 4D clusters: {avg_coverage:.1f}%",
