"""
Primary Benchmark test for 2D minimization (2-4 variables).
Tests BoolMin2D performance against PyEDA for small K-maps.
...
OUTPUTS_DIR = os.path.join(SCRIPT_DIR, "..", "outputs", "benchmark_results2D_pyeda")
RESULTS_CSV = os.path.join(OUTPUTS_DIR, "benchmark_results2D_pyeda.csv")
REPORT_PDF = os.path.join(OUTPUTS_DIR, "benchmark_scientific_report2D_pyeda.pdf")
STATS_CSV = os.path.join(OUTPUTS_DIR, "benchmark_results2D_pyeda_statistical_analysis.csv")
...
def calculate_statistics(pyeda_data, kmap_data, metric_name="metric"):
    ...
    Args:
        ...
        kmap_data: List of BoolMin2D measurements
    ...
    # Calculate differences (PyEDA - BoolMin2D)
...
def output_values_to_kmap(output_values, num_vars):
    """
    Convert linear output_values array to 2D K-map format for BoolMin2D.
    
    Args:
        output_values: List of 0, 1, or 'd' values (truth table format)
        num_vars: Number of variables (2, 3, or 4)
        
    Returns:
        2D list representing K-map in vranesic convention (default)
    """
    if num_vars == 2:
        # 2x2 K-map: rows=x2, cols=x1
        # Index order: x1x2 -> 00, 01, 10, 11
        return [
            [output_values[0], output_values[2]],  # x2=0: x1=0,1
            [output_values[1], output_values[3]]   # x2=1: x1=0,1
        ]
    elif num_vars == 3:
        # 2x4 K-map: rows=x3, cols=x1x2 (Gray code: 00,01,11,10)
        # Index order: x1x2x3
        # BoolMin2D uses: col_label + row_label = x1x2 + x3
        # So col positions map to: 00->0, 01->2, 11->6, 10->4 (for x3=0)
        #                          00->1, 01->3, 11->7, 10->5 (for x3=1)
        return [
            [output_values[0], output_values[2], output_values[6], output_values[4]],  # x3=0
            [output_values[1], output_values[3], output_values[7], output_values[5]]   # x3=1
        ]
    elif num_vars == 4:
        # 4x4 K-map: rows=x3x4 (Gray: 00,01,11,10), cols=x1x2 (Gray: 00,01,11,10)
        # Index order: x1x2x3x4
        gray_row = [0, 1, 3, 2]  # Gray code for rows (x3x4)
        gray_col = [0, 1, 3, 2]  # Gray code for cols (x1x2)
        kmap = []
        for row_idx in gray_row:
            row = []
            for col_idx in gray_col:
                # Calculate truth table index: x1x2x3x4
                idx = (col_idx << 2) | row_idx
                row.append(output_values[idx])
            kmap.append(row)
        return kmap
    else:
        raise ValueError(f"Unsupported number of variables: {num_vars}")
...
def test_single_case(num_vars, output_values, test_name):
    ...
    # BoolMin2D minimization with timing
    print("  BoolMin2D...", end=" ", flush=True)
    kmap_error = None
    try:
        # Convert output_values to 2D K-map format
        kmap_2d = output_values_to_kmap(output_values, num_vars)
        ...
        solver = BoolMin2D(kmap_2d)
        kmap_func = lambda: solver.minimize(form='sop')
    ...
    if pyeda_error is None and kmap_error is None:
        ...
        if is_constant:
            ...
            if (pyeda_is_zero and kmap_is_zero) or (pyeda_is_one and kmap_is_one):
                ...
                print(f"✗ FAIL [CONSTANT MISMATCH: PyEDA={pyeda_str}, BoolMin2D={kmap_str}]")
                equiv = False
        else:
            pyeda_mismatches = 0
            kmap_mismatches = 0
            ...
            if pyeda_mismatches > 0 or kmap_mismatches > 0:
                print(f"✗ FAIL (PyEDA: {pyeda_mismatches}, BoolMin2D: {kmap_mismatches} mismatches)")
                equiv = False
            else:
                print(f"✓ PASS")
                equiv = True
...
# Run tests
print("="*80)
print("BENCHMARK - 10 RANDOM FUNCTIONS EACH FOR 2-4 VARIABLES")
print("="*80)
...
for num_vars, output_size in configs:
    ...
    if valid_results:
        ...
        print(f"  BoolMin2D: {avg_kmap_time:.6f}s")
        print(f"  Speedup:   {speedup:.2f}x")
        
        print(f"\nAverage Literal Count:")
        print(f"  PyEDA:     {avg_pyeda_lits:.1f}")
        print(f"  BoolMin2D: {avg_kmap_lits:.1f}")
...
if pyeda_times and kmap_times:
    ...
    print(f"  BoolMin2D: avg={np.mean(kmap_times):.6f}s, min={np.min(kmap_times):.6f}s, max={np.max(kmap_times):.6f}s")
    speedup = np.mean(pyeda_times) / np.mean(kmap_times)
    print(f"  Speedup:   {speedup:.2f}x ({'BoolMin2D faster' if speedup > 1 else 'PyEDA faster'})")
...
if pyeda_lits and kmap_lits:
    ...
    print(f"  BoolMin2D: avg={np.mean(kmap_lits):.1f}, min={np.min(kmap_lits)}, max={np.max(kmap_lits)}")
    lit_ratio = np.mean(kmap_lits) / np.mean(pyeda_lits) if np.mean(pyeda_lits) > 0 else 1.0
    print(f"  Ratio:     {lit_ratio:.4f} ({'BoolMin2D more compact' if lit_ratio < 1 else 'PyEDA more compact' if lit_ratio > 1 else 'Equal'})")
...
# Perform statistical analysis for each variable count
print("\n" + "="*80)
...
for num_vars in sorted(results_by_var.keys()):
    ...
    # Filter valid results (where both succeeded and not constant)
    valid_results = [r for r in var_results if r['pyeda_time'] > 0 and r['kmap_time'] > 0]
    ...
    if valid_results:
        ...
        # Filter non-zero literals for simplification quality analysis
        literal_results = [r for r in valid_results if r['pyeda_literals'] > 0 and r['kmap_literals'] > 0]
        
        if literal_results:
            pyeda_lits = [r['pyeda_literals'] for r in literal_results]
            kmap_lits = [r['kmap_literals'] for r in literal_results]
        else:
            pyeda_lits = [0]
            kmap_lits = [0]
        
        # Calculate statistics
        time_stats = calculate_statistics(pyeda_times, kmap_times, "execution_time")
        lit_stats = calculate_statistics(pyeda_lits, kmap_lits, "literal_count")
        ...
        print(f"  Valid tests: {len(valid_results)}/10")
        print(f"  Non-constant tests: {len(literal_results)}/10")
        print(f"\n  TIMING:")
        print(f"    Mean diff (PyEDA - BoolMin2D): {time_stats['mean_diff']:+.6f}s")
        print(f"    95% CI: [{time_stats['ci_lower']:.6f}, {time_stats['ci_upper']:.6f}]")
        print(f"    Significant: {'YES' if time_stats['significant'] else 'NO'} (p={time_stats['p_value']:.6f})")
        print(f"    Effect size: {time_stats['cohens_d']:.4f} ({interpret_effect_size(time_stats['cohens_d'])})")
        
        if len(literal_results) > 0:
            print(f"\n  SIMPLIFICATION QUALITY:")
            print(f"    Mean literal diff (PyEDA - BoolMin2D): {lit_stats['mean_diff']:+.2f}")
            print(f"    Mean deviation: {abs(lit_stats['mean_diff']):.2f} literals")
            print(f"    95% CI: [{lit_stats['ci_lower']:.2f}, {lit_stats['ci_upper']:.2f}]")
            print(f"    Significant: {'YES' if lit_stats['significant'] else 'NO'} (p={lit_stats['p_value']:.6f})")
...
try:
    with PdfPages(REPORT_PDF) as pdf:
        ...
        bars2 = ax1.bar(x + width/2, time_means_kmap, width, label='BoolMin2D', alpha=0.8, color='#A23B72')
        ...
        bars2 = ax2.bar(x + width/2, lit_means_kmap, width, label='BoolMin2D', alpha=0.8, color='#A23B72')
...
Configurations Tested:   {len(all_stats)} (2, 3, 4 variables)
...
"""
        ...
        for num_vars in sorted(all_stats.keys()):
            ...
            winner_time = "BoolMin2D" if time_stats['mean_diff'] > 0 else "PyEDA" if time_stats['mean_diff'] < 0 else "TIE"
            winner_lit = "BoolMin2D" if lit_stats['mean_diff'] > 0 else "PyEDA" if lit_stats['mean_diff'] < 0 else "TIE"
...
EXECUTION TIME:
  ...
  Mean BoolMin2D:       {time_stats['mean_kmap']:.6f} s
...
SIMPLIFICATION QUALITY (Literal Count):
  ...
  Mean BoolMin2D:       {lit_stats['mean_kmap']:.2f} literals
...
1. PERFORMANCE:
   ...
   • Average BoolMin2D time: {overall_time_stats['mean_kmap']:.6f} s
...
2. SIMPLIFICATION QUALITY:
   ...
   • Average BoolMin2D literals: {overall_lit_stats['mean_kmap']:.2f}
...
"""
        ...
        for num_vars in sorted(all_stats.keys()):
            ...
            better = "BoolMin2D" if lit_stats['mean_diff'] > 0 else "PyEDA" if lit_stats['mean_diff'] < 0 else "Neither"
            conclusions += f"""
