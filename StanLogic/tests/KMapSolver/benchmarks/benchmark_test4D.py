"""
BoolMinGeo 4D Minimization: Performance Benchmark for 9-10 Variable Functions
==============================================================================

PERFORMANCE-ONLY STUDY: BoolMinGeo's 4D minimization method evaluation

This benchmark tests BoolMinGeo's minimize_4d() method, which extends geometric 
minimization into four dimensions for 9-10 variable Boolean functions. The study 
characterizes performance under various distribution patterns.

KEY FEATURES:
- Tests minimize_4d() method specifically (not minimize_3d())
- Variable range: 9-10 variables (512-1024 truth table entries)
- Five distribution types: sparse, dense, balanced, minimal DC, heavy DC
- Metrics: execution time, memory usage, solution quality (literals/terms)
- No SymPy comparison (computational infeasibility at this scale)

OUTPUTS:
- Raw performance data CSV for detailed analysis
- Statistical analysis CSV for paper writing (descriptive statistics)
- Comprehensive PDF report with visualizations

Author: Somtochukwu Stanislus Emeka-Onwuneme
Date: January 2026
Version: 6.0 (4D Minimization Benchmark)
"""
...
OUTPUTS_DIR = os.path.join(SCRIPT_DIR, "..", "outputs", "benchmark_results4D")
RESULTS_CSV = os.path.join(OUTPUTS_DIR, "benchmark_results4D.csv")
STATS_CSV = os.path.join(OUTPUTS_DIR, "benchmark_results4D_statistical_analysis.csv")
REPORT_PDF = os.path.join(OUTPUTS_DIR, "benchmark_results4D_performance_report.pdf")
...
def document_experimental_setup():
    ...
    setup_info = {
        ...
        "study_type": "BoolMinGeo 4D Minimization Performance (9-10 variables)",
    }
    return setup_info
...
def run_single_test(test_num, num_vars, output_values, dist_name):
    ...
    (terms, expr), time_s, mem_mb, peak_mem_mb = benchmark_with_resources(
        lambda: solver.minimize_4d(form='sop')
    )
...
def create_performance_report(all_results, pdf):
    ...
    plt.suptitle('BoolMinGeo 4D Minimization Performance Characterization (9-10 Variables)', 
                 fontsize=16, fontweight='bold', y=0.995)
    pdf.savefig(bbox_inches='tight')
    plt.close()
    print("✓")
...
def export_statistical_analysis(all_results, filename=STATS_CSV):
    """Export descriptive statistical analysis of BoolMinGeo 4D performance to CSV.
    
    Since this is a performance-only study (no benchmark comparison),
    exports descriptive statistics grouped by variable count and distribution.
    """
    print(f"\n📊 Exporting statistical analysis...", end=" ", flush=True)
    
    # Group results by configuration
    configs = sorted(set((r['num_vars'], r['distribution']) for r in all_results if not r['is_constant']))
    
    with open(filename, mode="w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        
        # Header
        writer.writerow(["Configuration", "Metric", "N", "Mean", "Std Dev", 
                        "Min", "Q1", "Median", "Q3", "Max", "CV%"])
        
        for num_vars, dist in configs:
            # Filter results for this configuration
            config_results = [r for r in all_results 
                            if r['num_vars'] == num_vars 
            ...
            config_name = f"{num_vars}-var {dist.upper()}"
            
            # Time statistics
            times = np.array([r['time_s'] for r in config_results])
            writer.writerow([
                config_name, "Time (s)",
                len(times),
                f"{np.mean(times):.8f}",
                f"{np.std(times, ddof=1):.8f}",
                f"{np.min(times):.8f}",
                f"{np.percentile(times, 25):.8f}",
                f"{np.median(times):.8f}",
                f"{np.percentile(times, 75):.8f}",
                f"{np.max(times):.8f}",
                f"{(np.std(times, ddof=1) / np.mean(times) * 100):.2f}" if np.mean(times) > 0 else "N/A"
            ])
            
            # Memory statistics
            mems = np.array([r['memory_mb'] for r in config_results])
            writer.writerow([
                config_name, "Memory (MB)",
                len(mems),
                f"{np.mean(mems):.2f}",
                f"{np.std(mems, ddof=1):.2f}",
                f"{np.min(mems):.2f}",
                f"{np.percentile(mems, 25):.2f}",
                f"{np.median(mems):.2f}",
                f"{np.percentile(mems, 75):.2f}",
                f"{np.max(mems):.2f}",
                f"{(np.std(mems, ddof=1) / np.mean(mems) * 100):.2f}" if np.mean(mems) > 0 else "N/A"
            ])
            
            # Peak Memory statistics
            peak_mems = np.array([r['peak_memory_mb'] for r in config_results])
            writer.writerow([
                config_name, "Peak Memory (MB)",
                len(peak_mems),
                f"{np.mean(peak_mems):.2f}",
                f"{np.std(peak_mems, ddof=1):.2f}",
                f"{np.min(peak_mems):.2f}",
                f"{np.percentile(peak_mems, 25):.2f}",
                f"{np.median(peak_mems):.2f}",
                f"{np.percentile(peak_mems, 75):.2f}",
                f"{np.max(peak_mems):.2f}",
                f"{(np.std(peak_mems, ddof=1) / np.mean(peak_mems) * 100):.2f}" if np.mean(peak_mems) > 0 else "N/A"
            ])
            
            # Literal count statistics
            literals = np.array([r['num_literals'] for r in config_results])
            writer.writerow([
                config_name, "Literals",
                len(literals),
                f"{np.mean(literals):.2f}",
                f"{np.std(literals, ddof=1):.2f}",
                f"{np.min(literals):.0f}",
                f"{np.percentile(literals, 25):.2f}",
                f"{np.median(literals):.2f}",
                f"{np.percentile(literals, 75):.2f}",
                f"{np.max(literals):.0f}",
                f"{(np.std(literals, ddof=1) / np.mean(literals) * 100):.2f}" if np.mean(literals) > 0 else "N/A"
            ])
            
            # Term count statistics
            terms = np.array([r['num_terms'] for r in config_results])
            writer.writerow([
                config_name, "Terms",
                len(terms),
                f"{np.mean(terms):.2f}",
                f"{np.std(terms, ddof=1):.2f}",
                f"{np.min(terms):.0f}",
                f"{np.percentile(terms, 25):.2f}",
                f"{np.median(terms):.2f}",
                f"{np.percentile(terms, 75):.2f}",
                f"{np.max(terms):.0f}",
                f"{(np.std(terms, ddof=1) / np.mean(terms) * 100):.2f}" if np.mean(terms) > 0 else "N/A"
            ])
            
            # Time per entry (efficiency)
            time_per_entry = np.array([r['time_per_entry_ms'] for r in config_results])
            writer.writerow([
                config_name, "Time/Entry (ms)",
                len(time_per_entry),
                f"{np.mean(time_per_entry):.6f}",
                f"{np.std(time_per_entry, ddof=1):.6f}",
                f"{np.min(time_per_entry):.6f}",
                f"{np.percentile(time_per_entry, 25):.6f}",
                f"{np.median(time_per_entry):.6f}",
                f"{np.percentile(time_per_entry, 75):.6f}",
                f"{np.max(time_per_entry):.6f}",
                f"{(np.std(time_per_entry, ddof=1) / np.mean(time_per_entry) * 100):.2f}" if np.mean(time_per_entry) > 0 else "N/A"
            ])
    
    print(f"✅ Done ({len(configs)} configurations)")
...
def main():
    print("="*80)
    print("BoolMinGeo 4D Minimization Performance Characterization (9-10 Variables)")
    print("="*80)
    ...
    print(f"✓ Raw data CSV exported: {RESULTS_CSV}")
    
    # Export statistical analysis
    export_statistical_analysis(all_results)
    print(f"✓ Statistical analysis CSV exported: {STATS_CSV}")
    ...
    with PdfPages(REPORT_PDF) as pdf:
        ...
        ax.text(0.5, 0.52, '4D Minimization Performance', fontsize=24, ha='center')
        ax.text(0.5, 0.45, '9-10 Variable Boolean Functions', fontsize=18, ha='center')
...
To reproduce this experiment:
  ...
  4. Execute: python benchmark_test4D.py
"""
