"""
Dense 16-Variable 4D EPI Analysis with QM Merging
===================================================

Analyzes 4D EPI reduction through Quine-McCluskey merging for dense
16-variable K-maps. Dense functions have 90% of cells as 1s.
...
# Number of test cases (reduced for 16-var due to size: 65536 cells)
NUM_TEST_CASES = 100
...
RESULTS_CSV = os.path.join(OUTPUTS_DIR, "dense_16var_4d_epi_results.csv")
STATS_TXT = os.path.join(OUTPUTS_DIR, "dense_16var_4d_epi_stats.txt")
HISTOGRAM_PNG = os.path.join(OUTPUTS_DIR, "dense_16var_4d_epi_histogram.png")

# ============================================================================
# TEST DATA GENERATION
# ============================================================================

def random_dense_output_values_16var():
    """
    Generate a dense random 16-variable output values list.
    Dense means 90% of cells are 1s, 10% are 0s.
    
    For 16 variables, we need 2^16 = 65536 output values.
    
    Returns:
        List of 65536 values (0 or 1) with ~90% being 1
    """
    size = 2**16  # 65536 cells
...
def analyze_dense_kmaps_4d_epis():
    """
    Generate dense 16-variable K-maps and analyze 4D EPI reduction through QM merging.
    ...
    print(f"Generating and analyzing {NUM_TEST_CASES} dense 16-variable K-maps...")
    print("=" * 70)
    print("Note: Each test processes 65536 cells (2^16 variables)")
    print("Extracting 4D EPIs and applying QM merge...")
    print("=" * 70)
    
    for test_num in range(1, NUM_TEST_CASES + 1):
        # Generate dense output values
        output_values = random_dense_output_values_16var()
        
        # Count ones
        num_ones = sum(1 for v in output_values if v == 1)
        density = num_ones / 65536.0
        
        # Create solver
        solver = BoolMinGeo(16, output_values)
...
def create_histogram(results, stats):
    ...
    fig.suptitle(f'Dense 16-Variable Functions (N={len(results)}, {stats["mean_density"]:.1%} density)', 
...
def save_statistics_txt(stats, results):
    ...
    with open(STATS_TXT, 'w') as f:
        f.write("Dense 16-Variable 4D EPI Analysis with QM Merging\n")
        f.write("=" * 70 + "\n\n")
        
        f.write(f"Configuration:\n")
        f.write(f"  Random Seed: {RANDOM_SEED}\n")
        f.write(f"  Number of Test Cases: {NUM_TEST_CASES}\n")
        f.write(f"  Function Type: Dense 16-variable ({DENSITY_MIN:.0%}-{DENSITY_MAX:.0%} density)\n")
        f.write(f"  K-Map Size: 65536 cells (2^16)\n\n")
        ...
        f.write("-" * 70 + "\n")
        f.write(f"1. 16-variable K-maps have 2^16 = 65536 cells\n")
...
def main():
    ...
    print("\n" + "=" * 70)
    print("DENSE 16-VARIABLE 4D EPI ANALYSIS WITH QM MERGING")
    print("=" * 70)
    print(f"Random Seed: {RANDOM_SEED}")
    print(f"Test Cases: {NUM_TEST_CASES}")
    print(f"K-Map Size: 65536 cells (2^16)")
